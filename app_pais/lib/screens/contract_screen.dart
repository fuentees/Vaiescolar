  ...
  Future<void> _load() async {
    ...
    if (mounted) {
      if (contracts.isNotEmpty && _name.text.trim().isEmpty) {
        ...
        final responsible =
            (current['responsible_name'] ?? '').toString().trim();
  ...
  Future<void> _sign(Map<String, dynamic> contract) async {
    ...
    if (missing.isNotEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Revise os campos destacados para assinar.')));
      return;
    }
    FocusScope.of(context).unfocus();
    setState(() => _saving = true);
    final error = await Api.signContract(
        contract['id'] as String, _name.text.trim(), cpf, _password.text, code);
    if (!mounted) return;
              ...
              : SafeArea(
                  ...
                  child: ListView(
                      ...
                      children: [
                        ...
                        Text(
                            '${widget.studentName} · versao ${current['version']}'),
                        ...
                        Card(
                          ...
                          child: SelectableText(
                              current['contract_text'] as String,
                        ...
                        if (expired) ...[
                          const Card(
                            ...
                            subtitle: Text(
                                'Solicite ao transportador uma nova emissao.'),
                          )),
                          ...
                          if (_showValidation && !_accepted)
                            const Padding(
                              padding: EdgeInsets.only(bottom: 10),
                              child: Text('Marque o aceite para continuar.',
                          ...
                          TextField(
                            ...
                            decoration: const InputDecoration(
                                labelText:
                                    'Nome completo de quem esta assinando',
                                prefixIcon: Icon(Icons.draw_outlined)),
                            onChanged: (_) => setState(() {}),
                          ),
                          if (_showValidation && _name.text.trim().length < 3)
                            const Padding(
                              padding: EdgeInsets.only(top: 6),
                              child: Text('Informe o nome completo.',
                                  style: TextStyle(color: Colors.red)),
                            ),
                          const SizedBox(height: 12),
                          TextField(
                          ...
                          if (_showValidation &&
                              _cpf.text.replaceAll(RegExp(r'\D'), '').length !=
                                  11)
                            const Padding(
                              padding: EdgeInsets.only(top: 6),
                              child: Text('O CPF deve conter 11 dígitos.',
                                  style: TextStyle(color: Colors.red)),
                            ),
                          const SizedBox(height: 12),
                          TextField(
                          ...
                          if (_showValidation && _password.text.isEmpty)
                            const Padding(
                              padding: EdgeInsets.only(top: 6),
                              child: Text(
                                  'Digite a senha usada para entrar no app.',
                                  style: TextStyle(color: Colors.red)),
                            ),
                          const SizedBox(height: 12),
                          ...
                          TextField(
                            ...
                            decoration: const InputDecoration(
                              ...
                              hintText: '000000',
                              helperText:
                                  'Toque em “Enviar código” e confira a notificação.',
                              counterText: '',
                          ...
                          if (_showValidation &&
                              _code.text.replaceAll(RegExp(r'\D'), '').length !=
                                  6)
                            const Padding(
                              padding: EdgeInsets.only(top: 6),
                              child: Text(
                                  'Informe exatamente os 6 dígitos enviados.',
                                  style: TextStyle(color: Colors.red)),
                            ),
                          const SizedBox(height: 10),
                          SizedBox(
